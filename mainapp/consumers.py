...
class StockConsumer(AsyncWebsocketConsumer):
    ...
    def addToCeleryBeat(self, stockpicker):
        ...
        if len(task)>0:
            ...
            for x in stockpicker:
                if x not in args:
                    args.append(x)
            task.args = json.dumps([args])              #dump the datat in celery beat arguments
            task.save()
        else:
    ...
    async def connect(self):
        ...
        quey_params = parse_qs(self.scope["query_string"].decode())
        print(quey_params)

        stockpickerr = quey_params['stockpicker']

        #add to celery beat
        await self.addToCeleryBeat(stockpickerr)

        #add user to StockDetail(db) #orm
        await self.addToStockDetail(stockpickerr)
    ...
    def helper_func(self):
        user = self.scope["user"]
        stocks = StockDetail.objects.filter(id=user.id)
        ...
        for i in stocks:
            i.user.remove(user)
            if i.user.count() == 0:
                stocks_to_remove.append(i.stock)
                i.delete()
        # args = [x for x in args if x not in stocks_to_remove]
